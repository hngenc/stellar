...
// The matmul layers of a ResNet50 inference
...
#error "we haven't added padding yet"
#endif

// TODO we shouldn't just repeat all the matmul code from large/matmul.c here
...
int main() {
    ...
    static elem_t left[MAT_DIM_I][MAT_DIM_K] stellar_align;
    static elem_t right[MAT_DIM_K][MAT_DIM_J] stellar_align;
    static acc_t bias[MAT_DIM_J] stellar_align;
    static elem_t result[MAT_DIM_I][MAT_DIM_J] stellar_align;
    ...
    // Calculate golden matrix
    static elem_t golden[MAT_DIM_I][MAT_DIM_J];
    for (int i = 0; i < MAT_DIM_I; i++)
        for (int j = 0; j < MAT_DIM_J; j++) {
            golden[i][j] = bias[j];
            for (int k = 0; k < MAT_DIM_K; k++)
    ...
    printf("DOUBLE_BUFFERED_AB: %d, DOUBLE_BUFFERED_CD: %d\n", DOUBLE_BUFFERED_AB, DOUBLE_BUFFERED_CD);
    printf("DIM: %d\n", DIM);
    ...
    // Print stats
    printf("Cycles: %llu\n", end-start);
    printf("Utilization: %llu%%\n", ((uint64_t)100 * (uint64_t)IDEAL_CYCLES) / (uint64_t)(end-start));
